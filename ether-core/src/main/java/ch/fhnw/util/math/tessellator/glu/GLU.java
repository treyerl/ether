package ch.fhnw.util.math.tessellator.glu;

import ch.fhnw.util.math.tessellator.glu.tessellator.GLUtessellatorImpl;
...
public class GLU {
	...
	/*****************************************************************************
	 * <b>gluDeleteTess</b> destroys the indicated tessellation object (which
	 * was created with {@link #gluNewTess gluNewTess}).
	 *
	 * Optional, throws GLException if not available in profile
	 *
	 * @param tessellator
	 ...
	 * are interpreted and rendered. The legal value for <i>which</i> are as
	 * follows:
	 * <P>
	 *
	 * Optional, throws GLException if not available in profile
	 ...
	 * <em>value</em> may be set to one of <BR>
	 * <b>GLU_TESS_WINDING_ODD</b>, <BR>
	 * <b>GLU_TESS_WINDING_NONZERO</b>, <BR>
	 * <b>GLU_TESS_WINDING_POSITIVE</b>, or <BR>
	 * <b>GLU_TESS_WINDING_NEGATIVE</b>, or <BR>
	 * <b>GLU_TESS_WINDING_ABS_GEQ_TWO</b>.
	 * <P>
	 *
	 ...
	 * be replaced by a single vertex. The tolerance is multiplied by the
	 ...
	 * the edge the vertex lies on. Two edges are merged only when both
	 ...
	 * interpreted and rendered. See the {@link #gluTessProperty
	 * gluTessProperty} reference page for information about the properties and
	 * what they do.
	 *
	 * Optional, throws GLException if not available in profile
	 *
	 ...
	 *            values are <b>GLU_TESS_WINDING_RULE</b>,
	 *            <b>GLU_TESS_BOUNDARY_ONLY</b>, and <b>GLU_TESS_TOLERANCES</b>.
	 * @param value
	 *            Specifices an array into which the value of the named property
	 *            is written.
	 *
	 * @see #gluNewTess gluNewTess
	 ...
	 * it is replaced. If <i>aCallback</i> is null, then the existing callback
	 * becomes undefined.
	 * <P>
	 *
	 * Optional, throws GLException if not available in profile
	 ...
	 * that the polygonData parameter used by some of the methods is a copy of
	 * the reference that was specified when {@link #gluTessBeginPolygon
	 ...
	 * <UL>
	 * The same as the <b>GLU_TESS_BEGIN</b> callback except that it takes an
	 * additional reference argument. This reference is identical to the opaque
	 * reference provided when {@link #gluTessBeginPolygon gluTessBeginPolygon}
	 ...
	 * <UL>
	 * The edge flag callback is similar to glEdgeFlag. The method takes a
	 ...
	 * opaque reference provided when {@link #gluTessBeginPolygon
	 * gluTessBeginPolygon} was called. The method prototype for this callback
	 * is:
	 * </UL>
	 *
	 * <PRE>
	 ...
	 * similar to glVertex3f, and it defines the vertices of the triangles
	 * created by the tessellation process. The method takes a reference as its
	 * only argument. This reference is identical to the opaque reference
	 * provided by the user when the vertex was described (see
	 * {@link #gluTessVertex gluTessVertex}). The method prototype for this
	 * callback is:
	 * </UL>
	 *
	 * <PRE>
	 ...
	 * of a primitive and it takes no arguments. The method prototype for this
	 * callback is:
	 * </UL>
	 *
	 * <PRE>
	 ...
	 * {@link #gluTessEndPolygon gluTessEndPolygon} is called.
	 * <P>
	 *
	 ...
	 * {@link #gluErrorString gluErrorString} call. The method prototype for
	 * this callback is:
	 * </UL>
	 *
	 * <PRE>
	 ...
	 * normal. This reference is passed back to the user through the
	 * <b>GLU_TESS_VERTEX</b> or <b>GLU_TESS_VERTEX_DATA</b> callback after
	 * tessellation (see the {@link #gluTessCallback gluTessCallback} reference
	 * page).
	 *
	 * @param tessellator
	 *            Specifies the tessellation object (created with
	 ...
	 * @param data
	 *            Specifies an opaque reference passed back to the program with
	 *            the vertex callback (as specified by {@link #gluTessCallback
	 *            gluTessCallback}).
	 *
	 * @see #gluTessBeginPolygon gluTessBeginPolygon
	 * @see #gluNewTess gluNewTess
	...
	/*****************************************************************************
	 * <b>gluTessBeginPolygon</b> and {@link #gluTessEndPolygon
	 * gluTessEndPolygon} delimit the definition of a convex, concave or
	 * self-intersecting polygon. Within each <b>gluTessBeginPolygon</b>/
	 * {@link #gluTessEndPolygon gluTessEndPolygon} pair, there must be one or
	 * more calls to {@link #gluTessBeginContour gluTessBeginContour}/
	 * {@link #gluTessEndContour gluTessEndContour}. Within each contour, there
	 * are zero or more calls to {@link #gluTessVertex gluTessVertex}. The
	 * vertices specify a closed contour (the last vertex of each contour is
	 * automatically linked to the first). See the {@link #gluTessVertex
	 * gluTessVertex}, {@link #gluTessBeginContour gluTessBeginContour}, and
	 * {@link #gluTessEndContour gluTessEndContour} reference pages for more
	 * details.
	 * <P>
	 *
	 * Optional, throws GLException if not available in profile
	 ...
	 * appropriate callback(s) are specified (see {@link #gluTessCallback
	 * gluTessCallback}), then this reference is returned to the callback
	 * method(s). Thus, it is a convenient way to store per-polygon information.
	 * <P>
	 *
	 * Once {@link #gluTessEndPolygon gluTessEndPolygon} is called, the polygon
	 * is tessellated, and the resulting triangles are described through
	 * callbacks. See {@link #gluTessCallback gluTessCallback} for descriptions
	...
	/*****************************************************************************
	 * <b>gluTessBeginContour</b> and {@link #gluTessEndContour
	 * gluTessEndContour} delimit the definition of a polygon contour. Within
	 * each <b>gluTessBeginContour</b>/ {@link #gluTessEndContour
	 * gluTessEndContour} pair, there can be zero or more calls to
	 * {@link #gluTessVertex gluTessVertex}. The vertices specify a closed
	 * contour (the last vertex of each contour is automatically linked to the
	 * first). See the {@link #gluTessVertex gluTessVertex} reference page for
	 * more details. <b>gluTessBeginContour</b> can only be called between
	...
	/*****************************************************************************
	 * <b>gluTessEndContour</b> and {@link #gluTessBeginContour
	 * gluTessBeginContour} delimit the definition of a polygon contour. Within
	 * each {@link #gluTessBeginContour gluTessBeginContour}/
	 * <b>gluTessEndContour</b> pair, there can be zero or more calls to
	 * {@link #gluTessVertex gluTessVertex}. The vertices specify a closed
	 * contour (the last vertex of each contour is automatically linked to the
	 * first). See the {@link #gluTessVertex gluTessVertex} reference page for
	 * more details. {@link #gluTessBeginContour gluTessBeginContour} can only
	 * be called between {@link #gluTessBeginPolygon gluTessBeginPolygon} and
	 * {@link #gluTessEndPolygon gluTessEndPolygon}.
	 *
	 * Optional, throws GLException if not available in profile
	 *
	...
	/*****************************************************************************
	 * <b>gluTessEndPolygon</b> and {@link #gluTessBeginPolygon
	 * gluTessBeginPolygon} delimit the definition of a convex, concave or
	 * self-intersecting polygon. Within each {@link #gluTessBeginPolygon
	 ...
	 * {@link #gluTessCallback gluTessCallback} for descriptions of the callback
	 * functions.
	 *
	 * @param tessellator
	 *            Specifies the tessellation object (created with
